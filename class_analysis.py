...
class DataAnalysis:
    ...
    def store_variables(ID, inputs):
        ...
        x_index, i_index, o_index, x, i, o, p, q = (
                inputs['index']['x'],    
                inputs['index']['i'],
                inputs['index']['o'],
                inputs['x'],
        ...
        df.to_csv("results/variables_x.csv") 
        ...
        df.to_csv("results/variables_i.csv") 
        ...
        for tup in tups:
            ...
            if q[tank, line, prod, j].X > 0 and o[tank, line, prod, j].X > 0:
                ...
                data.append(row)
        df = pd.DataFrame(data, columns=['Tank', 'Line', 'Product', 'Time', 'Volume'])
        df.to_csv("results/variables_o.csv") 
    ...
    def schedule(ID, inputs):
        ...
        x_index, i_index, o_index, x, i, o, p, q, CycleStart, T = (
                inputs['index']['x'],    
                inputs['index']['i'],
                inputs['index']['o'],
                inputs['x'],
        ...
        #
        data2 = inputs["fact_LineSchedule"] 
        data2 = data2.groupby(['Line', 'Product']).agg({'Hourly_Vol': 'sum'}).reset_index()
        data2.rename(columns={'Hourly_Vol': 'Volume_actual'}, inplace=True)
        ...
        # 
        #
        b = inputs["fact_tanks"][['Tank', 'Volume']]
        b.rename(columns={'Volume': 'Volume_Exists'}, inplace=True)
        ...
        # 
        #
        schedule.to_csv("results/opt_schedule.csv", index=False)
    ...
    def process_tank_volumes():
        ...
        # Read the csv file
        schedule = pd.read_csv("results/opt_schedule.csv")
    ...
    def process_line_volumes():
        ...
        df = (pd.read_csv("results/opt_schedule.csv")
